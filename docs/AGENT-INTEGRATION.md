...
Themis è pensato perché un agente (Claude Code, Cursor, o qualunque altro) lo chiami durante il
...
- **Codici di uscita stabili** (`themis/exitcodes.py`, contratto pubblico): `0` conforme, `1`
...
| Setup | nessuno (entry point `themis`) | `pip install themis-toolkit[mcp]`, un grafo di default |
...
1. **Prima di scrivere codice**, chiedi il brief del work package invece di rileggere l'intero
   grafo: `themis brief grafo.yaml WP-009` (CLI) o il tool MCP `brief`. Contiene requisiti da
...
3. **Prima di aprire la PR**, verifica la tracciabilità inversa sui soli file toccati (non
   ...
   themis trace-code grafo.yaml --code-dir . --files /tmp/cambiati.txt
   ```
...
4. **Se il work package tocca il grafo stesso** (nuovo requisito, WP, ADR, ...), ripeti
   ...
   ```bash
   themis fix-plan grafo.yaml --format json --watch --interval 2
   ```
...
[`skills/themis-authoring/`](../skills/themis-authoring/SKILL.md) guida l'estrazione e itera
`validate`/`fix-plan` automaticamente fino a zero difetti bloccanti. Non è pensata per giudicare
la conformità di un grafo già scritto da altri — per quello basta `themis validate` da solo.
...
(`themis/_build_marker.py`), esattamente come per la CI di questo stesso repo
...
"Contents: read" limitato a `themis-toolkit`, e salvarlo come secret `THEMIS_TOOLKIT_TOKEN` nel
...
`themis/license.py`), calcolate da `uuid.getnode()` — stabile su una macchina reale, non
...
- **Runner self-hosted dal cliente** (macchina/VM persistente che lui controlla): funziona come
  un suo workstation qualunque — `themis license fingerprint` una volta sul runner, la fingerprint
  autorizzata nella sua licenza (dentro i suoi `max_seats`), il binario e il file di licenza
  installati sul runner (o iniettati da un secret), poi `themis validate grafo.yaml --format
  json` come qualunque altro step CI. `themis ci-init --graph grafo.yaml` genera in locale il
  file `.github/workflows/themis.yml` con questo stesso step già pronto (`runs-on: self-hosted`,
...
- **Runner effimero (GitHub-hosted, ecc.)**: non supportato oggi — nessun modo di autorizzare in
  ...
  `ROADMAP.md`, F7.T4.5.
- **Nessun runner CI del cliente (né self-hosted né altro)**: `themis hooks-init [--graph
  ...
  da `hooks-init` stesso; anche disponibile come `themis init --with-hooks` per chi parte da
