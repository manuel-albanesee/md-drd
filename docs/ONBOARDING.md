...
2. **Scarica** il binario per la tua piattaforma (Windows/macOS/Linux) dalla stessa pagina, oppure
   direttamente dalle [release del repo pubblico `themis`](https://github.com/manuel-albanesee/themis/releases/latest)
   in qualunque momento (pubbliche, nessun account richiesto):

   | Eseguibile | A cosa serve |
   |---|---|
   | `themis` | CLI — comandi da terminale, script, CI |
   | `themis-mcp` | Server MCP — tool nativi per Claude Code, Cursor o un client MCP |

3. Rendili eseguibili e raggiungibili da terminale (su Linux/macOS: `chmod +x themis` e spostali
...
Senza una licenza valida per la macchina corrente, `themis --version` funziona ma **ogni altro
comando viene rifiutato** — è normale, il prossimo passo è attivare la licenza.

## 2. Attivazione della licenza

La verifica a runtime resta **sempre interamente offline** (`themis validate`/`init`/... non
...
```text
┌──────────┐  activation token  ┌──────────────────┐  themis license activate   ┌──────────────┐
│ Pagamento│ ──────────────────▶│  Pagina di        │ ───────────token+fpr─────▶│ themis-       │
...
1. Dopo il pagamento (§1), la pagina di consegna mostra un **activation token** e il comando da
   ...
   ```bash
   themis license activate --token <token>
   ```

2. Il CLI calcola la fingerprint di questa macchina, contatta l'unico endpoint di rete di tutto il
   toolkit (`themis-licensing`, provisioning una tantum — non un controllo a ogni uso), riceve il
   file di licenza firmato (Ed25519), lo scrive in `~/.themis/license.json` e lo riverifica subito
...
4. Posizionalo come `~/.themis/license.json`, oppure indicane il percorso con la variabile
   d'ambiente `THEMIS_LICENSE_FILE` (utile per non toccare la home, o in CI):

   ```bash
   export THEMIS_LICENSE_FILE=/percorso/al/license.json
   ```
...
`THEMIS-SPEC-001` (bisogno → requisito → architettura → work package → test), validato dagli
stessi 73 controlli.
...
1. **Scegli dove vive il grafo** dentro il repository esistente — una cartella dedicata, non
   mescolata al codice applicativo, es. `requisiti/` o `themis/`:

   ```bash
   cd /percorso/del/tuo/progetto        # repo git già esistente
   themis init requisiti/ --project "Nome Progetto" --code ACME
   ```
...
2. **Se hai già requisiti in un documento** (analisi funzionale, verbale, backlog) non
   ...
   - un template `.tex` FSD/TSD → `themis import-tex --doc D1=Analisi.tex --out frammento.yaml`
   ...
   - una volta congelato un documento sorgente nel grafo, `themis verify-sources` ti avvisa se
...
themis init . --project "Nome Progetto" --code ACME --with-hooks
themis validate graph.yaml -v
```

`themis init` genera lo scheletro più piccolo già conforme (uno stakeholder per categoria, un
...
aggiungerli in un secondo momento, `themis hooks-init` fa la stessa cosa su un progetto già
inizializzato. Da lì:
...
2. Quando un work package è pronto per essere implementato, genera il suo brief invece di far
   ...
   ```bash
   themis brief graph.yaml WP-001 --out WP-001-brief.md
   ```
...
|---|---|
| `themis validate graph.yaml -v` | schema + 73 controlli su 10 gate; primo comando da lanciare su qualunque grafo |
| `themis fix-plan graph.yaml` | difetti raggruppati per controllo, bloccanti prima, con remediation già inclusa |
| `themis brief graph.yaml WP-005` | brief autosufficiente per implementare un work package |
| `themis project graph.yaml --out out/` | proietta RTM, report di conformità, registro di governance, backlog, ReqIF |
| `themis rules --check G3.2 --detailed` | motivazione, remediation ed esempio di un singolo controllo |
...
| | CLI (`themis`) | Server MCP (`themis-mcp`) |
|---|---|---|
...
```bash
# Claude Code
claude mcp add themis -- themis-mcp --graph /percorso/assoluto/a/graph.yaml
...
# Cursor: .cursor/mcp.json
{
  "mcpServers": {
    "themis": {
      "command": "themis-mcp",
      "env": { "THEMIS_GRAPH_PATH": "/percorso/assoluto/a/graph.yaml" }
    }
  }
}
```

Se `themis-mcp` non è nel `PATH`, usa il percorso completo dell'eseguibile scaricato al punto 1.
Dettagli (tool esposti, risorse, troncamento di elenchi grandi): [`docs/MCP.md`](MCP.md).

**Non devi scegliere una volta per tutte**: una persona può lanciare `themis fix-plan` a mano dal
...
git commit -m "requisiti: baseline iniziale Themis"
```
...
- **Cita gli id del grafo** nei commit e nel codice (`WP-009`, `REQ-042`, ...): è lo stesso
  formato libero già usato dentro il grafo per ADR e requisiti, e `themis trace-code` lo legge
...
- **Prima di aprire una PR**, verifica che le citazioni nel codice puntino a id reali e non ad
  ...
  themis trace-code graph.yaml --code-dir . --files cambiati.txt
  ```

- **I documenti proiettati** (`out/`, generati da `themis project`) sono derivati, non scritti a
...
- **Hook git nativi sulla tua macchina**, se non hai (ancora) una CI:
  ...
  ```bash
  themis hooks-init --graph requisiti/graph.yaml
  ...
  è una rete di sicurezza, non un controllo imposto. Fatto in un colpo solo con `themis init
...
- **In CI**, il codice di uscita decide l'esito della build senza bisogno di fare parsing
  ...
  - **Validi il grafo con il toolkit installato da sorgente** (nessuna licenza richiesta): un
    ...
    jobs:
      validate:
        uses: manuel-albanesee/themis-toolkit/.github/workflows/validate-graph.yml@master
        with:
          graph-path: requisiti/graph.yaml
        secrets:
          toolkit-token: ${{ secrets.THEMIS_TOOLKIT_TOKEN }}
    ```
  ...
  - **Fai girare il binario licenziato sulla tua CI** (un runner self-hosted persistente, non
    ...
    ```bash
    themis ci-init --graph requisiti/graph.yaml
    ```

    genera `.github/workflows/themis.yml` già pronto per `runs-on: self-hosted`, con lo step
    `themis validate` incluso e nessun riferimento al sorgente privato del toolkit. Commettilo
    così com'è.
...
- **"Comando rifiutato" su tutto tranne `license fingerprint`** → nessuna licenza installata, o
  installata nel posto sbagliato. Verifica `themis license info` e la variabile
  `THEMIS_LICENSE_FILE` se non usi il percorso di default.
- **Licenza scaduta** → per design nessun periodo di tolleranza (verifica offline, niente server
  ...
  locale risulta scaduta (rinnovo non ancora propagato), rilancia `themis license activate
...
- **Macchina non autorizzata / ho spostato il progetto su un'altra macchina** → la fingerprint
  cambia, la vecchia licenza non è valida sulla nuova macchina. Rilancia `themis license activate
  ...
  manuale.
- **`themis validate` non trova lo schema** → non capita col binario compilato (schema
